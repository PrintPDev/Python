...
Durch die Erstellung von Parent-Klassen und Child-Klassen kann es oft sehr schwierig werden
welche Klasse von welche anderen Klasse geerbt wird. 

-> Um die Erblinie zu kontrollieren bietet Python zusätzlich zur type()-Funktion auch die
Funktion isinstance(). 
...
def main():
    benz_cla = Fahrzeug('Mercedes', 'CLA45', 385)
    fiat_500 = PKW('Fiat', '500', 75, '2-Türer', sitzplaetze=2, kilometerstand=25000)
    audi_q8 = SUV('Audi', 'Q8', 240, 'Sline', 1000, offroad_faehigkeit=False, sitzplaetze=4)
    ford_raptor = Pickup('Ford', 'Raptor', 320, '', 1000, sitzplaetze=4)

    # Wie ist hier die Erblinie? Wie kann ich diese kontrollieren?
    print('Was ist der Typ von den Objekten: ', type(benz_cla)) # Alle Objekte überprüfen
    print('Ist der Ford Raptor ein Kind von SUV oder PKW: ', isinstance(ford_raptor, (SUV, PKW))) # Bei SUV -> False
    print('Ist der SUV ein Kind von PKW bzw. Fahrzeug: ', isinstance(ford_raptor, Fahrzeug))
